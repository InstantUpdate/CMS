...
class Process extends CS_Controller
{
    ...
    public function uri()
    {
        ...
        foreach ($blocks as $block) {
            ...
            if ($content->exists())
                $block->setAttribute('data-id', $content->id);
            ...
            //mark as global
            if (!empty($content->is_global))
                $classes[] = "iu-global";

            $block->class = implode(' ', $classes);

            //if can edit
            if (!empty($this->user) && $this->user->can_edit_content($content))
...
//*/
  ...
  (function() {
    ...
    var s = document.getElementsByTagName('script')[0]; s.parentNode.insertBefore(ga, s);
        ...
        //filter page through plugins
        $page = filter('process.page', $page);
    ...
    {
        $args = empty($args) ? func_get_args() : $args[0];
